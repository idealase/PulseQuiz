...
function TokenBadge({ value, estimated, label }: { value: number | null | undefined; estimated?: boolean; label: string }) {
  ...
  return (
    <span className="inline-flex items-center gap-1 text-xs rounded px-1.5 py-0.5 bg-white/5 text-white/70">
...
function CallRow({ call }: { call: AICallRecord }) {
  ...
  return (
    <div className="border-b border-gray-800">
      <div
        className="flex items-center gap-2 px-3 py-1.5 cursor-pointer hover:bg-white/[0.03] text-sm"
      ...
      >
        ...
        {isMulti && <span className="text-purple-300 text-xs">{m.total_calls} calls</span>}
        <span className="text-white/40 text-xs">{m.model}</span>
        <div className="flex-1" />
        <span className="text-green-300 text-xs font-mono">{formatMs(elapsed)}</span>
        <TokenBadge value={usage?.total_tokens} estimated={usage?.estimated} label="tok" />
        <span className="text-gray-600">{expanded ? '▼' : '▶'}</span>
      </div>
      {expanded && (
        <div className="bg-gray-950/50 px-4 py-2 text-xs space-y-1.5">
          ...
          {isMulti && m.calls && (
            <div className="mt-2 border-t border-gray-800 pt-1.5">
              <div className="text-white/40 text-xs mb-1">Sub-calls:</div>
              {m.calls.map((sub, i) => (
                <div key={i} className="flex gap-3 text-xs text-white/50 ml-2">
...
function SystemInfoPanel() {
  ...
  return (
    <div className="p-3 text-sm space-y-3 overflow-y-auto" style={{ maxHeight: '30vh' }}>
      {/* Backend system info */}
      <div>
        <div className="flex items-center justify-between mb-1">
          <span className="text-white/50 uppercase tracking-wider text-xs">Backend System</span>
          <button onClick={refreshDevInfo} disabled={devInfoLoading} className="text-xs text-cyan-400 hover:text-cyan-300 disabled:text-gray-600">
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          <span className="text-white/50 uppercase tracking-wider text-xs">Backend Token Usage (24h)</span>
          <button onClick={() => refreshTokenSummary(24)} disabled={tokenSummaryLoading} className="text-xs text-cyan-400 hover:text-cyan-300 disabled:text-gray-600">
        ...
        {tokenSummary ? (
          <div className="space-y-1">
            ...
            {tokenSummary.endpoint_breakdown && Object.keys(tokenSummary.endpoint_breakdown).length > 0 && (
              <div className="mt-1">
                <span className="text-white/40 text-xs">By endpoint: </span>
                {Object.entries(tokenSummary.endpoint_breakdown).map(([ep, count]) => (
                  <span key={ep} className="inline-flex items-center gap-1 mr-2 text-xs">
    ...
    <div>
      ...
      <div className="flex items-center gap-2 px-3 py-1 border-b border-gray-800">
        ...
        {tab === 'calls' && sessionTotals.totalCalls > 0 && (
          <button onClick={clearCalls} className="text-xs text-gray-500 hover:text-red-400 px-1">clear</button>
        )}
      </div>
